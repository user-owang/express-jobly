...
class User {
  ...
  /** Creates new application with {username, jobID}
   *
   * Returns { jobID }
   *
   * Throws BadRequestError on duplicates.
  ...
  static async apply({ username, jobID }) {
    ...
    if (userCheck.rows.length === 0 && jobCheck.rows.length === 0) {
      throw new NotFoundError(
        `User: ${username} and job: ID#${jobID} not found`
      );
    } else if (userCheck.rows.length === 0) {
      throw new NotFoundError(`User: ${username} not found`);
    } else if (jobCheck.rows.length === 0) {
