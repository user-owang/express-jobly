...
describe("POST /users/:username/jobs/:jobID", function () {
  test("works for user", async function () {
    const resp = await request(app)
      .post(`/users/u1/jobs/${c2CEO}`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({ applied: c2CEO });
  });
  ...
  test("unauth for wrong user", async function () {
    const resp = await request(app)
      .post(`/users/u3/jobs/${c2CEO}`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
  });

  test("unauth for anon", async function () {
    const resp = await request(app).post(`/users/u1/jobs/${c2CEO}`);
    expect(resp.statusCode).toEqual(401);
  });

  test("not found if user missing", async function () {
    const resp = await request(app)
      .delete(`/users/nope/jobs/${c2CEO}`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(404);
  });
